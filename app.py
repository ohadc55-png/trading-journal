...
# --- חיבור למסד הנתונים (ללא Cache כדי למנוע התנגשויות) ---
def init_connection():
    try:
...
# --- שחזור Session (החלק שפותר את השגיאה שלך) ---
    ...
    with t1:
        ...
        if st.button("Log In", type="primary"):
            try:
                ...
                st.session_state.session = res.session # שמירת ה-Session!
                st.rerun()
...
def open_trade_modal():
    ...
    if st.button("Open Position", type="primary", use_container_width=True):
        ...
        try:
            # כאן היה הכישלון קודם - עכשיו זה יעבוד כי שחזרנו את ה-Session למעלה
...
# בעתיד: לולאה על כל הטריידים הסגורים לחישוב PnL מצטבר
